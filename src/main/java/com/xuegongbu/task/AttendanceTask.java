...
@Slf4j
@Component
public class AttendanceTask implements CommandLineRunner {
    ...
    private void setupAttendanceTasksForToday() {
        ...
        try {
            ...
            for (Integer classNumber : allClassNumbers) {
                ...
                // 定义考勤时间点（课程开始后的第5分钟、第25分钟、第40分钟）
                List<Integer> attendanceMinutes = Arrays.asList(5, 25, 40);
                
                for (Integer minuteOffset : attendanceMinutes) {
                    // 计算考勤时间点
                    LocalTime attendanceTime = classStartTime.plusMinutes(minuteOffset);
                    
                    // 创建今天的完整时间
                    LocalDateTime taskTime = LocalDateTime.of(today, attendanceTime);
                    ...
                    if (isWithinOperationalHours(attendanceTime)) {
                        ScheduledFuture<?> task = taskScheduler.schedule(() -> {
                            try {
                                executeAutoAttendanceForClassPeriod(classNumber, minuteOffset);
                            } catch (Exception e) {
                                log.error("执行第 {} 节课第 {} 分钟的考勤任务时发生错误", classNumber, minuteOffset, e);
                            }
                        }, scheduledTime);
     ...
     * @param minuteOffset 课程开始后的分钟数（5, 25, 40）
     */
    private void executeAutoAttendanceForClassPeriod(int classPeriod, int minuteOffset) {
        log.info("开始执行第 {} 节课第 {} 分钟的自动考勤任务", classPeriod, minuteOffset);
        ...
        try {
            ...
            log.info("当前时间 {} 是第 {} 节课的第 {} 分钟，开始查询课程", now.toLocalTime(), classPeriod, minuteOffset);
            ...
            for (CourseSchedule course : ongoingCourses) {
                try {
                    performAutoAttendance(course, now, minuteOffset);
                } catch (Exception e) {
                    log.error("为课程 {} 执行自动考勤时发生错误: {}", course.getId(), e.getMessage(), e);
                }
            }

            log.info("第 {} 节课第 {} 分钟的自动考勤任务执行完成", classPeriod, minuteOffset);
        } catch (Exception e) {
            log.error("执行第 {} 节课第 {} 分钟的考勤任务时发生错误", classPeriod, minuteOffset, e);
        }
    }
     ...
     * @param minuteOffset 课程开始后的分钟数（5, 25, 40）
     */
    private void performAutoAttendance(CourseSchedule course, LocalDateTime checkTime, int minuteOffset) {
        log.info("开始为课程 {} 执行第 {} 分钟的自动考勤", course.getId(), minuteOffset);
        ...
        attendance.setRemark("自动考勤 - 第" + minuteOffset + "分钟");
        attendanceService.updateById(attendance);
        
        log.info("课程 {} 第 {} 分钟自动考勤完成，实到 {} 人，应到 {} 人，出勤率 {}%", 
                course.getCourseName(), minuteOffset, attendance.getActualCount(), attendance.getExpectedCount(),
